...
const TSE_URLS = {
  presidente: 'https://cdn.tse.jus.br/estatistica/sead/odsele/votacao_candidato_munzona/votacao_candidato_munzona_2022.zip',
};
...
async function baixarArquivoTSE(url: string, filename: string): Promise<void> {
...
async function extrairZip(filename: string): Promise<void> {
  const filepath = path.join(TEMP_DIR, filename);
...
interface VotosPresidente {
  lula: number;
  bolsonaro: number;
  outros: number;
}

interface VotosGovernador {
  tarcisio: number;
  haddad: number;
  outros: number;
}

interface DadosVotacaoCidade {
  ...
  votosPresidente2022: VotosPresidente;
  votosGovernador2022: VotosGovernador;
  votosLegendaPSDB45: number;
  votosLegendaPSD55: number;
}

async function processarCSVVotacao(cidadesAlvo: Map<string, number>): Promise<Map<number, DadosVotacaoCidade>> {
  ...
  const dadosPorCidade = new Map<number, DadosVotacaoCidade>();
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(csvPath, { encoding: 'latin1' })
      ...
      .on('data', (row: any) => {
        ...
        if (cargo === '6') {
          // Paulo Alexandre - número 4545 ou nome contendo PAULO ALEXANDRE
          ...
          // Votos por partido
          if (partido === 'PSDB' || partido === '45') {
            dados.votosPSDBTotal2022 += votos;
          } else if (partido === 'PSD' || partido === '55') {
...
async function main() {
  ...
  const cidadesAlvo = new Map<string, number>();
  ...
  if (!spFileExists) {
    await baixarArquivoTSE(TSE_URLS.presidente, 'votacao_2022.zip');
    await extrairZip('votacao_2022.zip');
  } else {
